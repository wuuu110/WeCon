...
def sample_pomo_every4th_topk_else_random(
    ...
    if mask_top.any():
        q_top = q[:, mask_top, :]              
        fb_top = fallback[:, mask_top]         
        Pt = q_top.size(1)

        top_idx = torch.topk(q_top, k=k_eff, dim=-1, largest=True).indices  
...
class RF(nn.Module):
    ...
    def forward(self, attn_out, pref):
        ...
        x = torch.cat([attn_out, pref_expand], dim=-1)  
        delta = self.fc2(F.relu(self.fc1(x)))           
